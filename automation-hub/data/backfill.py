...
def deep_backfill(store: HistoricalStore, symbol: str, timeframe: str, *,
                  years: float = 3.0, fetcher: Callable = fetch_klines) -> dict:
    """Backfill ``years`` of real candles for one symbol/timeframe."""
...
class BackfillJob:
    ...
    def start(self, *, symbols=SYMBOLS, timeframes=DEFAULT_TIMEFRAMES,
        ...
        with self._lock:
            ...
            self.state = {"running": True, "years": years,
                          "started_at": datetime.now(timezone.utc).isoformat(),
                          "finished_at": None, "done": 0, "total": len(pairs),
                          "current": None, "results": []}
            self._thread = threading.Thread(
                target=self._run, args=(pairs, years, fetcher),
                name="backfill", daemon=True)
            self._thread.start()
        return {"started": True, **self.state}

    def _run(self, pairs, years: float, fetcher: Callable) -> None:
        for sym, tf in pairs:
            self.state["current"] = f"{sym} {tf}"
            t0 = time.time()
            try:
                res = deep_backfill(self.store, sym, tf, years=years, fetcher=fetcher)
